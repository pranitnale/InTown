  ...
  config :realtime, RealtimeWeb.Endpoint,
    ...
    http: [
      ...
      transport_options: [
        ...
        # DEV OVERRIDE (see docker-compose.dev.yml): [:inet6] upstream. This sandbox's
        # kernel has no IPv6 (/proc/sys/net/ipv6 absent), so an inet6 listener fails to
        # bind with :eafnosupport and crash-loops the container. Forced to IPv4 for the
        # local dev stack only; production keeps the vendored [:inet6] default.
        socket_opts: [:inet]
      ]
    ],
    check_origin: false,
